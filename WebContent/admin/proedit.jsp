...
<html>
	...
	<body>
		<form action="proedit" method="post" enctype="multipart/form-data">
			<p class="path">时光锦商城/产品中心/编辑产品</p>
			<table border="0" cellspacing="0" cellpadding="0" class="tb">
				<tr>
					<td class="lable">类别<%=objbyid.get("typeid")%></td><td>
					
					    <select name="typeid">
					    	<option <%=(objbyid.get("typeid").toString().equals("1"))?"selected=\"selected\"":""%> value="1">传统服饰</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("2"))?"selected=\"selected\"":""%> value="2">鞋类</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("3"))?"selected=\"selected\"":""%> value="3">礼品</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("4"))?"selected=\"selected\"":""%> value="4">饰品</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("5"))?"selected=\"selected\"":""%> value="5">生活周边</option>
					    	<option <%=(objbyid.get("typeid").toString().equals("6"))?"selected=\"selected\"":""%> value="6">人气商品</option>
