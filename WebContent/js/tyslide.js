			...
			$.fn.extend({
				tyslide:function(opts){
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						$(item).find(".btnleft").css({"opacity":"0.3"});//设置按钮的透明度
						$(item).find(".btnright").css({"opacity":"0.3"});//设置按钮的透明度
						...
						$(item).find(".controls>li").text("").css({ "width": prams.controlsW, "height": prams.controlsH, "border-radius": prams.radius, "padding": "0", "display": "block", "border": "0", "opacity": "0.9" });
						...
						$(item).mouseenter(function(){
							if(prams.isShowBtn)
							{
								$(item).find(".btnleft").css({"opacity":"0.9"});
								$(item).find(".btnright").css({"opacity":"0.9"});
							}							
						});
						...
						$(item).mouseleave(function(){
							if(prams.isShowBtn)
							{
								$(item).find(".btnleft").css({"opacity":"0.3"});
								$(item).find(".btnright").css({"opacity":"0.3"});
							}							
						});
